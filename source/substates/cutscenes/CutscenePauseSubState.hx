...
typedef ExitSignal = FlxTypedSignal<PauseExit->Void>;
...
class CutscenePauseSubState extends MusicBeatSubState
{
	...
	public function new(?finishCallBack:PauseExit->Void)
	{
		...
		for (i in 0...buttonNames.length)
		{
			...
			btn.frames = Assets.sparrow('ui/cutscenes/$name');
			var anims:Array<Array<Dynamic>> = (name != "skip") ? [['idle', true], ['click', false],] : [['idle', true], ['hold', false], ['hold loop', true], ['release', false],];
			for (j in anims)
	...
	public function moveButtons(moveIn:Bool)
	{
		...
		if (moveIn)
		{
			new FlxTimer().start(0.05, (_) -> lockControls = false);
            new FlxTimer().start(0.9, (_) -> lockMovement = false);
		}
	}
	...
	private function end(type:PauseExit, timer:Float = 0)
	{
		...
		if (timer > 0)
			new FlxTimer().start(timer, (_) -> exit(type));
		else
			exit(type);
	}
}
